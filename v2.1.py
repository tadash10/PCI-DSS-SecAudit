...
def check_ec2_instances(ssh_cidr_range, outdated_amis):
    ...
    for reservation in instances['Reservations']:
        for instance in reservation['Instances']:
            ...
            patching_issues = check_patching(instance_id, ssm)
...
def check_patching(instance_id, ssm):
    """Check if the instance is patched and up to date."""
    try:
        response = ssm.list_instance_patches(InstanceId=instance_id)
        patches = response.get('Patches', [])
        if not patches:
            logging.warning(f"Instance {instance_id} has no patches available.")
            return [f"Instance {instance_id} is not patched."]
        
        # Example check for missing patches
        missing_patches = [patch['Title'] for patch in patches if patch['InstallTime'] is None]
        if missing_patches:
            logging.warning(f"Instance {instance_id} is missing patches: {', '.join(missing_patches)}.")
            return [f"Instance {instance_id} is missing patches: {', '.join(missing_patches)}."]
    except Exception as e:
        logging.error(f"Failed to check patch status for instance {instance_id}: {e}")
        return [f"Error checking patch status for instance {instance_id}."]
...
def check_access_control(instance_id, iam):
    ...
    try:
        ...
        if not role:
            ...
            role_name = role['InstanceProfile']['Roles'][0]['RoleName']
            policies = iam.list_attached_role_policies(RoleName=role_name)
            for policy in policies['AttachedPolicies']:
                # Here you could analyze policy permissions
                if policy['PolicyName'] == 'AWSElasticBeanstalkWebTier':
                    issues.append(f"IAM role {role_name} has overly permissive policy: {policy['PolicyName']}.")

            logging.info(f"Instance {instance_id} has IAM role: {role_name}. Policies: {policies['AttachedPolicies']}.")
    except Exception as e:
